...
export function Funnel({ data }: { data: FunnelTotals }) {
  const steps = [
    { key: "Views", value: data.views, hint: "просмотры видео" },
    { key: "Reach", value: data.reach, hint: "уникальный охват" },
    { key: "Profile", value: data.profileVisits, hint: "заходы в профиль" },
    { key: "Link", value: data.linkClicks, hint: "клики по ссылке" },
  ];
    ...
    <div className="panel p-5 md:p-6">
      ...
      <div className="grid gap-3">
        {steps.map((s, i) => {
          ...
          return (
            <div key={s.key} className="grid grid-cols-[88px_1fr_auto] items-center gap-3">
              <div>
                <p className="text-xs font-medium text-fg-muted">{s.key}</p>
                <p className="text-[10px] text-fg-subtle">{s.hint}</p>
              </div>
              <div className="h-9 overflow-hidden rounded-[var(--radius-sm)] bg-bg">
                <div
                  className="h-full rounded-[var(--radius-sm)] bg-teal/25 transition-[width] duration-500"
                  style={{ width: `${w * 100}%` }}
                />
              </div>
              <div className="min-w-24 text-right">
                ...
                {i > 0 ? (
                  <p className="text-[10px] text-fg-subtle">{formatPct(conv)}</p>
                ) : null}
              </div>
            </div>
          );
